...
// Fallback coordinates (Irvine, CA) used when GPS is unavailable
const FALLBACK_COORDS = { lat: 33.643, lng: -117.841 };
...
export default function SearchScreen() {
  ...
  // ── Effect 1: load preferences from storage ──────────────────────────────
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  // ── Effect 2: get device location, fall back gracefully ───────────────────
  useEffect(() => {
    async function getInitialLocation() {
      try {
        ...
        // GPS unavailable in browser/simulator — use fallback so app still works
        console.warn("Location unavailable, using fallback:", err);
        setLocationName("Irvine, CA (approximate)");
        setCoords(FALLBACK_COORDS);
      }
    }
    getInitialLocation();
  ...
  // ── Effect 3: auto-search once we have both coords and prefs ─────────────
  // Fetch is inlined here (not via runSearch) to avoid a stale closure —
  // coords and prefs are guaranteed fresh when this effect fires.
  useEffect(() => {
  ...
  // ── Manual search (search bar + quick picks + recent) ────────────────────
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerButtons}>
            <Pressable onPress={() => router.push("/for-you")} style={styles.forYouButton}>
              <Text style={styles.forYouButtonText}>✨ For You</Text>
            </Pressable>
            <Pressable onPress={() => router.push("/personalization")} style={styles.prefButton}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {recent.length > 0 && (
            <>
              <Text style={[styles.sectionTitle, { marginTop: 18 }]}>Recent</Text>
            ...
            <View key={index} style={styles.resultCard}>
              <View style={styles.resultHeader}>
                ...
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={styles.distanceTag}>{item.distance_miles} mi</Text>
                  <Text style={styles.priceSubtext}>{"$".repeat(item.price_range)}</Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  forYouButtonText: { fontWeight: "700", color: "#fff", fontSize: 13 },
  prefButton: {
    paddingVertical: 10,
    paddingHorizontal: 12,
    borderRadius: 12,
  ...
  prefsLabel: { color: "#666", marginBottom: 6, fontWeight: "700", fontSize: 12 },
  ...
  resultTitle: { fontSize: 18, fontWeight: "800", flex: 1, paddingRight: 8 },
  priceSubtext: { fontSize: 12, color: "#27ae60", fontWeight: "bold", marginTop: 4 },
  distanceTag: {
